		...
		<div class="box-body">
			<form action="{{ route('admin.book.update', $book) }}" method="POST" enctype="multipart/form-data">
				...
				<div class="form-group @error('title') has-error @enderror">
					...
					<input type="text" name="title" class="form-control" placeholder="Masukkan nama buku" value="{{ $book->title ?? old('title') }}">
					@error('title')
				...
				<div class="form-group @error('description') has-error @enderror">
					...
					<textarea name="description" placeholder="Masukkan deskripsi buku">{{ $book->description ?? old('description') }}</textarea>
					@error('description')
						<span class="help-block">{{ $message }}</span>
					@enderror
				</div>
